Chapter 7: Arrays
=======
At this point in this book, we have discussed data as scalar data types.  Each data item we have discussed is an abstraction of a single word in memory.  Whether we need a integer, float or character, these items take up one memory word.
...
Arrays are collections of data where each data item is identical and arranged in such a way that they can be referenced with integer indicies.  Arrays in C start at index 0 and, once declared, are fixed in length.  
...
Once declared, individual data items can be referenced by their position in the collection.  References are formed from the array name with the position number in square brackets.  For example, we can reference items from the above array as

    arr[3] = 12;

Here, the fourth element of the collection is assigned the value `12`.  

Array references are just like variable references, except the memory word used is calculated from the index given in the reference. So these references work well:
...
While these references look somewhat random or arbitrary, the real genius of array comes in the ability to calculate an array reference, especially when we use array references in a for loop with a loop index.  For example, consider this code to compute a summation:

    for (int i=0; i<10; i++) sum += arr[i];

You can't do that with regular scalar variable names. 

The biggest danger with arrays is that references might reference an item from the array that's beyond the boundaries of the array.  This type of error is call an *out of bounds* reference.  For example, given the declaration above, the boundaries of the array are from index 0 to index 9.  Referencing index 10 would be an error, because it would be outside those boundaries.   
...
> 
Technically, the standard for the C language specifies out of bounds references as "undefined behavior," which means compilers can handle these any way they want.  The reality is that an out of bounds reference will access memory outside of the array being referenced.  It will either access other variables in the program or it will access memory from protected areas, like memory dedicated to other processes.  In the former case, such a reference constitutes a bug, changing memory in unintended ways.  In the latter case, the program will likely crash. 
>
...
As an algorithm, the Bubble Sort is not very good in terms of performance.  However, the example is an especially bad version of the Bubble Sort.  Exercise 7.1 will ask you to make it perform better.
...
We have seen that arrays are collections of identical items, adjacent to each other in memory, referenced by position.  We can put anything into an array.  We can even have arrays of arrays; these are called two-dimensional arrays.
...
Now, initialization to zero happens so often that you can use abbreviated initialization syntax to initialize to zero. You may leave out values and they will be assumed to be zero.  This works only when the *remaining* values after specific initialization are to be set to zero.  Here's an example:

    int arr[10] = {0, 1, 2};

In this example, all the elements of the `arr` array are initialized. Only elements 1 and 2 have non-zero values; all others are set to zero.  
...
The last element at the `array_length` point is now considered to have an invalid value.
...
Rewrite the example code to demonstrate each of these methods. 
...
Starting with the result of Project 6.2, make the following changes.

1. Change the string from Project 6.2 into an array of 5 strings, 3 numerals each.  Because a string is just an array of characters, you can refer to each character from the string with an array reference.  Therefore, you should change the string from Project 6.2 into a two-dimensional array.
2. Change the `draw_digit` function from Project 6.2 to handle this two-dimensional array.  You will have to change both the parameters and the function code.
...
Take look at the starter code for Project 7.3.  This code generates a maze in the two-dimensional array called `maze`.  Each element in the array is a 4-bit number that looks like this:

IMAGE

When the bit is on/1, there is an open passage in that direction.

This project ask you to do three things:

1. Draw the maze on the Pebble screen.  You can do this using ...
2. Solve the maze.  
3. Draw the solution on the Pebble screen.  Again, you will need ...
