...
In the example, we used a number of *operators* to manipulate values of variables.  These operators depend on the kind of value we are using (see the next section on *data types* for more), but the *assignment operator* is always defined for all variables.  The assignment operator takes the value on the right of the "=" sign and makes it the value of the variable on the left of the "=" sign.  When a variable is referenced on the right of the "=" operator, it gives up its value.  So when the third line in the above example is executed, it's the same as as executing `milesPerGallon = 230 / 12`. 
...
It is indeed possible to use a variable without first assigning it a value.  Remember that variables are simply abstractions for memory words.  The result of using a variable without initializing it is unpredictable; you get whatever was left over in memory when the program started executing.  It is best to assume that this left over data is just useless.

**Data Types**

Variables have two important properties: what *values* they have take on and what *operations* can be done to them.  Together, these two properties are called a *data type*.  Every variable used in a C program must have a data type.
...
Here, we have *declared* three variables: `radius`, which can take on integers, `pi`, which can take on floating point values, and `circumference`, which take on floating point values that are *double* the size of other floating point values.  A variable must be declared before it is used in a C program; the C compiler must know what values and operations are to be used with a variable before it works with that variable.

Once we have declared the variables as in the example above, we can work with them like this:
...
Since setting a variable to an initial value is such a common exercise, declarations allow assignments in the declaration statement.  For example, we can combine the above declaration and assignment examples like this:
...
> **Dynamic Data Typing**
>
There are other data type rules.  The direct opposite of static typing is *dynamic typing*.  In dynamic typing, variables change their data type depending on the values assigned to them.  Variables need no declaration because they have no initial data type; they take on a data type when values are assigned.  In the example above, the last line does not convert the result to double before assignment, the variable takes on a float data type to match the right hand expression's data type.  

In C, variables are also *strongly typed*.  This means that once variables are bound to a data type, they stay bound to that data type.  This means that they do not change data types, but require other data types to be converted to their data type before they are assigned.    
...
Here, we see the use of the `%` operator -- the modulo or remainder operator -- and we have to decide about division.  The modulo operator is an integer operator that produces the *remainder* of a division of the two operands.  In this case, the variable `angle` will increment, then get divided by `360`, with the remainder assigned back to `angle`.  If this code were repeated, the value of `angle` would cycle between 0 and 360.
...
<table>
<TR><TD><b>Literal</b></TD><TD><B>Description</B></TD><TD>Example</TD><TD>Illegal Example</TD></TR>
<TD>Integer</TD><TD>A whole number in decimal, octal, or hexadecimal form.  Integers can have prefixes that explain the base in which they are specified and suffixes that explain the type of the integer.</TD><TD>23<br/>23L<br/>0x23<br/>0o23L</TD><TD>23.0<br/>0o238<br/></TD></TR> 
...
The table in the previous sections adds two data type to ones we have discussed.  *Character* data types are declared with the `char` keyword and are designed to hold *character* values.  On a Pebble watch, these values come from the [Unicode](http://www.unicode.org/charts) character set.  Unicode contains a Western style alphabet in the first 128 characters and a large collection of the other alphabets in the rest of the set.  Note that character data types include numbers as characters, so that `'5'` is not the same as `5`.  Character literals are represented using single quotes.
...
The type in parentheses converts the immediately adjacent value, which is, in this case, `number`.  The result is a floating point value, because a floating point value (`number`) was included in the computation.  
...
A left-to-right evaluation of the expression would yield a value of `22`.  However, C has a different order of evaluation.  The rules that specify the order of evaluation are called *precedence rules*.  A simple set of precedence rules are given in the table below.
...
>
The truth is that we really don't have a reliable explanation as to where this came from.  However, it's the rule now, so we deal with it.

We need to consider one more set of rules for expression evaluation. *Associativity* is a property of operators in the C language.  For example, in the expression `15 - - 2` might seem at first glance to be illegal, until one realizes that the righthand `-` sign is a negation operator and is *associated* with the `2` in the expression.  Associativity plays a role in expression evaluation and is considered first before operator precedence.
...
We have mentioned that the assignment operator is indeed an *operator*.  This means that we can combine it with other operators in potentially confusing expressions. To make sense of this, we need to remember that, as an operator, assignment evaluates to the value from the right side of the `=`.
...
At this point in the chapter, we have seen some straightforward rules about C and expressions.  We have also seen some interesting and odd combinations of expression shortcuts, type conversions, and precedence rules.  If a programmer is not careful with the code she writes, a big mess could result.  The syntax rules are given for flexibility, but they can be exploited with some strange results.
...
Variables should be declared as close to their use as possible.  So avoid global variables and stick with local declarations.
...
The code in these functions are run when the top and bottom buttons are pressed, respectively.  Add some code to the `up_click_hanlder` function -- between the curly braces (`{}`) that will make the ball bigger when the button is pressed.  You can do it in one line.  Use the shorthand notation for incrementing a variable.
...
Every time this code is run (when the timer goes off), the angle is incremented by 1.  Notice that the modulo operator (`%`) is used, which advances the variable `angle` from 0 to 359, then starts over at 0. Also notice that the variable `dangle` is declared inside a block of code, showing that declarations don't always have to appear before code.  
